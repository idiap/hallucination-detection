...
""" Evaluates BART-GBP's word probabilities by computing the points on the precision/recall curve. """
...
def get_predictions(values, keys):
    all_values = [flatten(values[key]) for key in keys]
    return [max(probs) for probs in zip(*all_values)]
...
def main(args):
    ...
    # compute precision-recall and ROC curves for all hallucinations and only the intrinsic/extrinsic ones
    results = {}
    for hallucinations in ['intrinsic_hallucinations', 'extrinsic_hallucinations', 'all_hallucinations']:
        ...
        for pid, prediction in predictions.items():
            ...
            preds = get_predictions(prediction, args.statistics)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--predictions_path', default='results/frank/bart_gbp_predictions.jsonl')
    parser.add_argument('--output_path', default='results/frank/bart_gbp_results.json')
    parser.add_argument('--statistics', nargs='+', default=[
        'extrinsic_sentence_aligned', 'extrinsic_inverse_entropy'
    ])
